...
actions = function(api, cb) {
  ...
  api.actions._start = function(api, cb) {
    return next(cb);
  };
  api.actions._teardown = function(api, cb) {
    return next(cb);
  };
  api.actions.pre = function() {};
  api.actions.post = function() {};
  ...
  api.actions.loadAllActions = (function() {
    ...
    loadFolder = function(folder) {
      if (fs.existsSync(folder)) {
        return fs.readdirSync(folder).forEach(function(file) {
          var action, ext, fullFilePath, realPath, requireKey, stats, _ref;
          ...
          if (stats.isDirectory()) {
            ...
            _ref = file.split('.'), action = _ref[0], ext = _ref[1];
    ...
    return [path.resolve(__dirname, '../actions/'), path.resolve(api.project_root, 'actions/')].forEach(loadFolder);
  })();
  return next(cb);
};
