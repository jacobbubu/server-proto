...
streams = function(api, cb) {
  var _, _ref;
  _ref = api.utils, next = _ref.next, _ = _ref._;
  api.streams = {};
  api.streams.map = {};
  api.streams.loadStreamFile = function(file) {
    var e, innerFunc, streamName, _ref1, _results;
    try {
      _ref1 = require(file);
      _results = [];
      for (streamName in _ref1) {
        innerFunc = _ref1[streamName];
  ...
  api.streams.loadAllStreams = (function() {
    ...
    return _.uniq([path.resolve(__dirname, '../streams/'), path.resolve(api.project_root, 'streams/')]).forEach(loadFolder);
  })();
  return next(cb);
};
